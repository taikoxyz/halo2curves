...
fn msm(c: &mut Criterion) {
    ...
    // Todo:  Modify this to enforce single
    let singlecore_engine = H2cEngine::new();
    for k in SINGLECORE_RANGE {
        group
            .bench_function(BenchmarkId::new("singlecore", k), |b| {
                assert!(k < 64);
                let n: usize = 1 << k;
                b.iter(|| singlecore_engine.msm(&coeffs[..n], &bases[..n]));
            })
            .sample_size(10);
    }
    // Todo:  Modify this to enforce ulticore
    let multicore_engine = H2cEngine::new();
    for k in MULTICORE_RANGE {
        group
            .bench_function(BenchmarkId::new("multicore", k), |b| {
                ...
                let n: usize = 1 << k;
                b.iter(|| {
                    multicore_engine.msm(&coeffs[..n], &bases[..n]);
                })
            })
