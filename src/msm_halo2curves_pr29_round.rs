...
pub struct Round<C: CurveAffineExt> {
    t: Vec<C>,
    odd_points: Vec<C>,
...
macro_rules! second_phase {
    ($out:expr, $p0:expr, $p1:expr, $acc:expr) => {
        *$p1.mut_y() = $p1.y() * $acc;
        $acc = $acc * $p1.x();
        *$out.mut_x() = $p1.y().square() - $out.x();
        *$out.mut_y() = ($p1.y() * ($p0.x() - $out.x())) - $p0.y();
    };
}


impl<C: CurveAffineExt> Round<C> {
    pub(crate) fn new(n_buckets: usize, n_points: usize) -> Self {
        let odd_points = vec![C::identity(); n_buckets];
        let bucket_sizes = vec![0; n_buckets];
        //  TODO: requires less than allocated
        let t = vec![C::identity(); n_points * 2];
        Self {
            t,
            odd_points,
            bucket_sizes,
    ...
    pub(crate) fn init(&mut self, bases: &[C], positions: &[u32], bucket_sizes: &[usize]) {
        {
        ...
        for (bucket_index, bucket_size) in bucket_sizes.iter().enumerate() {
            ...
            if bucket_size_pre == 0 {
                self.odd_points[bucket_index] = C::identity();
...
#[cfg(test)]
mod tests {
    use crate::msm_halo2curves_pr29::get_data;
    use super::Round;
