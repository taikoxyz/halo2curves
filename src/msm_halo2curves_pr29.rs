use crate::msm::multiexp_serial;
use ff::PrimeField;
use crate::arithmetic::CurveAffineExt;
use crate::msm_halo2curves_pr29_round::Round;
...
pub struct MSM<C: CurveAffineExt> {
...
impl<C: CurveAffineExt> MSM<C> {
    pub fn allocate(n_points: usize, override_window: Option<usize>) -> Self {
        ...
        let n_windows = div_ceil!(C::Scalar::NUM_BITS as usize, window);
    ...
    fn decompose(&mut self, scalars: &[C::Scalar]) {
    ...
    pub fn evaluate(scalars: &[C::Scalar], points: &[C], acc: &mut C::Curve) {
        Self::evaluate_with(scalars, points, acc, None)
    }

    pub fn best(scalars: &[C::Scalar], points: &[C]) -> C::Curve {
        ...
        if scalars.len() > num_threads {
            ...
            multicore::scope(|scope| {
                    ...
                    .zip(results.iter_mut())
                {
                    scope.spawn(move |_| {
                        if points.len() < 1 << 8 {
                            multiexp_serial(scalars, points, acc);
                        } else {
...
///
////////////////////////////////////////////////////////////////
// Not in module as we need public get_data
// also can't reuse generate_coefficients_and_curvepoints
// as it generates exceptional points that this MSM can't handle

#[cfg(test)]
use crate::bn256::{G1, G1Affine, Fr};
#[cfg(test)]
use ff::Field;
#[cfg(test)]
use group::Curve;
...
#[cfg(test)]
pub fn get_data(n: usize) -> (Vec<G1Affine>, Vec<Fr>) {
...
fn test_msm() {
    ...
    for k in min_k..=max_k {
        ...
        multiexp_serial(scalars, points, &mut r0);
        let mut r1 = G1::identity();
        MSM::evaluate(scalars, points, &mut r1);
        assert_eq!(r0, r1);
    }
}
